...
//  tft.setTextColor(ILI9340_WHITE);
  ...
  interruptSetup1();                 // sets up to read Pulse Sensor signal every 2mS  
}
...
void loop(){
  ...
  if (QS1 == true){                       // Quantified Self flag is true when arduino finds a heartbeat
    ...
    countdigits[2] = BPM1 % 10;
    //How to handle the middle digit depends on if the
    //the speed is a two or three digit number
    if(BPM1 > 99){
      countdigits[1] = (BPM1 / 10) % 10;
    }
    else{
      countdigits[1] = BPM1 / 10;
    }
    //Grab the first digit
    countdigits[0] = BPM1 / 100;
    ...
    if(BPM1 != prevCount){
      ...
      for(x=0; x < 3; x++){
        if(countdigits[x] != prevdigits[x]){
          ...
          Serial.println(prevdigits[x]);
          //print new value in white
          if((x == 0) and (BPM1 > 99) and (countdigits[x] > 0)){
            //tft.setCursor(digitpos[x], 70);
            //tft.setTextColor(ILI9340_WHITE);
            Serial.println(countdigits[x]);
          }
          ...
          else if((x == 1) and (BPM1 < 99) and (countdigits[x] > 0)){
            //tft.setCursor(digitpos[x], 70);
            //tft.setTextColor(ILI9340_WHITE);
            Serial.println(countdigits[x]);
          }
      ...
      prevCount = BPM1; //Store current bpm for comparison on the next loop.
    } 
